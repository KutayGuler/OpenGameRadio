		...
		typeof Object.assign == 'function'
			...
			: function (a, b) {
					for (var c = 1; c < arguments.length; c++) {
						var d = arguments[c];
						if (d) for (var g in d) E(d, g) && (a[g] = d[g]);
					}
					return a;
				};
			...
			if (m && E(f[0], l))
				for (f = 0; f < m.length; f++) {
					...
					if ((h !== h && n.key !== n.key) || h === n.key)
						return { id: l, list: m, index: f, o: n };
				}
...
*/
	...
	q.D = function (a) {
		...
		if (b) {
			...
			this.m != 0
				...
				: (c && xa(c, a), delete this.g[a], delete this.g[a + 1], delete this.g[a + 2]);
		}
		return !!b;
	};
	q.H = function (a, b) {
			...
			if (this.A)
				...
				try {
					for (g = 0, k = c.length; g < k && !this.i; g++)
						(e = c[g]), this.g[e + 1].apply(this.g[e + 2], d);
				} finally {
	...
	function pb(a, b, c) {
		if (Array.isArray(b)) for (var d = 0; d < b.length; d++) pb(a, String(b[d]), c);
	...
	function Db(a, b) {
		...
		window.location.href &&
			wa(['debugjs', 'debugcss'], function (f) {
		...
		W.yt_embedsEnableIframeSrcWithIntent
			? ($a(c, Wa(a)), c.sandbox.add('allow-presentation', 'allow-top-navigation'))
			: (c.src = a);
		return c;
	}
	q.G = function () {
	...
	function Ob(a) {
		if (a.tagName.toLowerCase() !== 'iframe') {
			var b = zb(a, 'videoid');
			b && ((b = { videoId: b, width: zb(a, 'width'), height: zb(a, 'height') }), new X(a, b));
		}
	}
	function Kb(a, b, c) {
		Z ||
				...
				a: if (Nb.has(d.origin)) {
					...
					if (k && d.origin === k.O)
						switch (
							((d = k.V),
							(d.m = !0),
							d.m && (wa(d.l, d.sendMessage, d), (d.l.length = 0)),
							(k = g.event),
							(g = g.info),
							k)
						) {
							case 'apiInfoDelivery':
								if (G(g)) for (var e in g) g.hasOwnProperty(e) && (d.v[e] = g[e]);
								break;
							case 'infoDelivery':
								Lb(d, g);
								break;
							case 'initialDelivery':
								...
								break;
							default:
								d.i.i || ((e = { target: d, data: g }), d.i.H(k, e), Ab('player.' + k, e));
						}
				}
			}));
		Z[b] = { V: a, O: c };
		...
		if (c.querySelectorAll && c.querySelector) b = c.querySelectorAll('.yt-player');
			...
			if (b.querySelectorAll && b.querySelector) b = b.querySelectorAll('.yt-player');
				...
				for (c = d = 0; (b = g[c]); c++) {
					var e = b.className,
						f;
					if ((f = typeof e.split == 'function')) f = va(e.split(/\s+/), 'yt-player') >= 0;
					f && (k[d++] = b);
				}
				k.length = d;
				b = k;
